...
import { iterateAll } from '../../src/collections/iteration/iterate.js';
import { MultiMap } from '../../src/collections/multi-map/multi-map.js';
import { Node } from '../../src/node.js';
import { nodesToItems } from '../../src/node-conversion.js';
import { Nodes } from '../../src/nodes.js';
import { traverseGraph } from '../../src/traversal/traverse-graph.js';
	...
	it('should signal the correct depth during traversal', () => {
		const actual = traverseGraph({
			...
			signal: (node, signal) => {
				...
				let expectedDepth = node.item == 0 ? 0 : Math.floor(Math.log10(node.item) + 1);
